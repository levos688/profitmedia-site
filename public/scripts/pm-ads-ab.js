(function () {
  var STORAGE_KEY = 'pm_ads_ab_v3';
  var SKIP_KEY = 'pm_ab_skip';
  var CTA_EXPERIMENTS = ['sticky_cta', 'bottom_copy', 'mid_cta', 'offer_cta'];
  ...
  var CONFIG = {
      ...
      variants: [
        {
          ...
          title: 'מערכת שמחברת פרסום לעסקאות',
          subtitle:
            'אני מחבר מחקר מתחרים, קמפיינים, דף נחיתה ו־CRM — עם פידבק לפרסום — כדי שהתקציב יביא פניות שמתקדמות לעסקה, לא רק טפסים.',
          cta: 'קבלו אבחון קמפיין חינם',
          ctaNote: 'אבחון חינם תוך 24 שעות · בלי התחייבות · בלי הבטחות מכירות',
          proofChip: '₪200K+ במכירות שדווחו בחודש · לקוח בתחום המטבחים',
        ...
        },
        {
          ...
          title: 'מערכת שמחברת פרסום לעסקאות',
          subtitle:
            'במקום לידים זולים שלא עונים — אני בונה מערכת: מחקר מתחרים ומסר, בדיקת קהלים ומודעות, דף שנבדק, ו־CRM עם פידבק לאיכות — עד העסקה.',
          cta: 'קבלו אבחון קמפיין חינם',
          ctaNote: 'אבחון חינם תוך 24 שעות · בלי התחייבות · בלי הבטחות מכירות',
          proofChip: '₪200K+ במכירות שדווחו בחודש · לקוח בתחום המטבחים',
          trustLine: 'עד 46% ירידה בעלות לליד · SKI VIP · 223+ לקוחות',
    ...
    sticky_cta: {
      variants: [
        ...
        { id: 'opt_plan', label: 'קבלו תוכנית אופטימיזציה אישית' },
      ],
    },
    popup_delay: {
      variants: [
    ...
    bottom_copy: {
      variants: [
        ...
        },
        {
          id: 'opt_plan',
          eyebrow: 'ללא כל התחייבות',
          title: 'השאירו פרטים וקבלו תוכנית אופטימיזציה אישית',
          submitLabel: 'קבלו תוכנית אופטימיזציה אישית',
        },
      ],
    },
    mid_cta: {
      variants: [
        ...
        {
          id: 'opt_plan',
          title: 'רוצים תוכנית אופטימיזציה אישית לתקציב שלכם?',
          subtitle: '3 פרטים קצרים · תוכנית אישית תוך 24 שעות · בלי התחייבות',
          button: 'קבלו תוכנית אופטימיזציה אישית',
        },
      ],
    },
    offer_cta: {
      variants: [
        ...
        {
          id: 'opt_plan',
          title: 'הצעד הראשון: תוכנית אופטימיזציה אישית — חינם',
          items: [
            'איפה הפרסום מפספס הזדמנויות לעסקאות',
            'מה לשפר קודם בקמפיינים ובדף',
  ...
  function applyForceOverrides(assignments) {
    try {
      ...
      var map = {
        ...
        force_bottom: 'sticky_cta',
        force_mid: 'sticky_cta',
        force_offer: 'sticky_cta',
      ...
      syncCtaFamily(assignments);
      if (changed) writeAssignments(assignments);
    } catch (e) {}
    return syncCtaFamily(assignments);
  }

  /** sticky_cta is master door CTA — keep bottom/mid/offer on the same id */
  function syncCtaFamily(assignments) {
    var ctaId = assignments.sticky_cta;
    if (!ctaId || !isActiveVariant('sticky_cta', ctaId)) return assignments;
    for (var i = 0; i < CTA_EXPERIMENTS.length; i++) {
      var exp = CTA_EXPERIMENTS[i];
      if (isActiveVariant(exp, ctaId)) assignments[exp] = ctaId;
    }
    return assignments;
  }
  ...
  function getAssignments() {
    ...
    if (existing) {
      ...
      syncCtaFamily(existing);
      if (changed) writeAssignments(existing);
      else writeAssignments(existing);
      return applyForceOverrides(syncCtaFamily(existing));
    }
    var assignments = {};
    Object.keys(CONFIG).forEach(function (experiment) {
    ...
    return applyForceOverrides(syncCtaFamily(assignments));
  }
  ...
  function isFacebookAdTraffic() {
    try {
      if (sessionStorage.getItem('pm_ads_fb_ad') === '1') return true;
      var url = new URL(window.location.href);
  ...
  function markFacebookAdTraffic() {
    ...
    try {
      sessionStorage.setItem('pm_ads_fb_ad', '1');
    } catch (e) {}
  }
  ...
  function applyHeroCopy(variantId, ctaId) {
    ...
    var sticky = findVariant('sticky_cta', ctaId || 'diagnosis');
    if (sticky && sticky.label) {
      setText(document.querySelector('[data-ab-hero-cta]'), sticky.label);
      setText(
        document.querySelector('[data-ab-hero-note]'),
        sticky.id === 'opt_plan'
          ? 'תוכנית אישית תוך 24 שעות · בלי התחייבות · בלי הבטחות מכירות'
          : copy.ctaNote,
      );
    } else {
  ...
  function applyAll(assignments) {
    syncCtaFamily(assignments);
    applyHeroCopy(assignments.hero_copy, assignments.sticky_cta);
    applySticky(assignments.sticky_cta);
    applyBottomCopy(assignments.sticky_cta);
    applyMidCta(assignments.sticky_cta);
    applyOfferCta(assignments.sticky_cta);
  }
  ...
  function openHomePopup(reason) {
    ...
    if (reason !== 'sticky') {
      ...
      try {
        if (sessionStorage.getItem('pm_ads_popup_auto_seen') === '1') return;
      } catch (e) {}
    }
    ...
    if (reason === 'timer' || reason === 'scroll') {
      try {
        sessionStorage.setItem('pm_ads_popup_auto_seen', '1');
      } catch (e2) {}
      popupEvents.push(
  ...
  function rebalanceAssignmentsIfNeeded() {
    ...
    Object.keys(CONFIG).forEach(function (experiment) {
      ...
      var flagKey = 'pm_ads_rebalance_' + experiment + '_' + (alloc.mode || 'x') + '_' + (alloc.leader || 'none');
      try {
  ...
  function loadAllocation() {
    return fetch('/api/ads-ab-allocate', { credentials: 'same-origin' })
      .then(function (res) {
        return res.json();
      })
      .then(function (data) {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function () {
      window.pmAdsAb.ready.then(function () {
        window.pmAdsAb.init();
      });
    });
  } else {
