(() => {
  ...
  const DAILY_SINCE_FALLBACK = '2026-07-26';
  const STATS_API = '/api/ads-ab-stats';
  ...
  const labels = {
    hero_copy: {
      system: 'V2 — מחקר מתחרים / רשימה ברורה',
      pain: 'V4 — לידים זולים שלא עונים',
    },
    sticky_cta: {
      diagnosis: 'קבלו אבחון קמפיין חינם',
      opt_plan: 'קבלו תוכנית אופטימיזציה אישית',
      order_system: 'הזמנת מערכת (retired)',
      start_work: 'התחלת עבודה (retired)',
    },
    popup_delay: {
      '20s': '20 секунд',
      '25s': '25 секунд',
    ...
    bottom_copy: {
      diagnosis: 'אבחון — форма внизу',
      opt_plan: 'תוכנית אופטימיזציה — форма внизу',
      order_system: 'הזמנת מערכת — форма (retired)',
    },
    mid_cta: {
      diagnosis: 'Mid CTA: אבחון',
      opt_plan: 'Mid CTA: תוכנית אופטימיזציה',
      order_system: 'Mid CTA: הזמנת מערכת (retired)',
    },
    offer_cta: {
      diagnosis: 'Offer block: אבחון',
      opt_plan: 'Offer block: תוכנית אופטימיזציה',
      order_system: 'Offer block: הזמנת מערכת (retired)',
    },
    lead_source: {
      simple: 'Форма внизу',
      popup_quiz: 'Popup с вопросом',
      popup_simple: 'Popup со sticky',
      mid_cta: 'Mid-page CTA → modal',
      offer_cta: 'Offer CTA → modal',
      sticky: 'Sticky → modal',
      hero_cta: 'Hero CTA → modal',
    },
  };

  const titles = {
    hero_copy: 'Hero subtitle: V2 clear vs V4 pain',
    sticky_cta: 'Door CTA: אבחון vs תוכנית אופטימיזציה',
    popup_delay: 'Таймер popup',
    popup_scroll: 'Скролл popup',
    bottom_copy: 'Нижняя форма (synced with sticky)',
    mid_cta: 'Mid-page CTA (synced with sticky)',
    offer_cta: 'Блок offer (synced with sticky)',
    lead_source: 'Источники заявок',
  };
  ...
  const channelNotes = {
    fb_ads: 'Показана статистика только с переходов из Facebook Ads (fbclid, utm_source=ri_XX и т.п.). Веса трафика считаются по этому же каналу.',
    all: 'Показана вся статистика, включая тестовые визиты. Важно: веса трафика (70/30 и т.п.) всегда считаются по Facebook Ads, а не по этой таблице — цифры показов могут не совпадать с текстом «показов a/b».',
  };
        ...
        return `<section class="ab-stats__block">
          ...
          ${
            showLegend && cachedData?.allocation?.[experiment.experiment]
              ? `<p class="ab-stats__alloc-note">${cachedData.allocation[experiment.experiment].reason}.${
                  activeChannel === 'all'
                    ? ' Веса — по Facebook Ads (lifetime), не по столбцам таблицы «все».'
                    : ''
                } Бейджи «Лучший / Слабый» — по лестнице сигнала (CVR × лиды). При solo слабый получает 0% трафика. Строка «Итого» считает варианты из активного списка.</p>`
              : isLeadSource
  ...
  async function loadStats() {
    ...
    try {
      ...
      const res = await fetch(`${STATS_API}?${query.toString()}`, { credentials: 'same-origin' });
      const contentType = res.headers.get('content-type') || '';
      if (!contentType.includes('application/json')) {
        if (statusEl) {
          statusEl.textContent =
            'API статистики недоступен здесь (нужен Cloudflare / прод). Откройте /ads/ab-stats на profitmedia.co.il с ?token=…';
        }
        return;
      }
        ...
        if (statusEl) {
          statusEl.textContent =
            data.error === 'Unauthorized'
              ? 'Нет доступа: добавьте ?token=… в URL (тот же токен, что у Donhin ab-stats)'
              : data.error || 'Не удалось загрузить статистику';
        }
        return;
      }
      ...
      if (statusEl) {
        statusEl.textContent =
          'Ошибка загрузки статистики' + (err && err.message ? `: ${err.message}` : '');
      }
    } finally {
      loading = false;
    }
